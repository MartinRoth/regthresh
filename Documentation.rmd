...
```{r GeneralSimulationSettigns, cache=TRUE}
sites <- 16
years <- 50
blockLength <- 92
...
```{r CreateXpar, cache=TRUE, dependson='GeneralSimulationSettings'}
xpar <- list()
xpar$S <- sites
xpar$T <- days
...
```{r SetMarginalParameters, cache=TRUE, dependson='CreateXpar'}
tau     <- 0.95
epsilon <- 0.25 
gamma   <- 0.5
xi      <- 0.15
...
```{r GenerateData, cache=TRUE, dependson='SetMarginalParameters'}
...
```{r declustering, cache=TRUE, dependson='LoadRainData'}
...
```{r ThresholdStabilityPlotAtSite, cache=TRUE, dependson=c('ObtainTSPlot', 'declustering')}
...
```{r ThresholdStabilityPlotRegional, cache=TRUE, dependson=c('ObtainTSPlot', 'declustering')}
