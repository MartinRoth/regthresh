...
\tikz[overlay, remember picture] \node[opacity=1, at=(current page.center)] {
   \includegraphics[height=\paperheight,width=\paperwidth]{AboutKNMI.png}};
}
\begin{frame}
\thispagestyle{empty}
\end{frame}
...
\begin{frame}{EVT related problems}
 \begin{textblock*}{5cm}(0.75cm, 3.5cm)\centering
  \begin{itemize}
   \item Combined extremes \\[3mm]%(Friesland example)
   \item Climate model validation \\[3mm]%(High resolution models / spatial extent)
   \item Improvement of extreme weather predictions \\[3mm]%(Jasper \& Juan)
   \item Earthquake analysis
  \end{itemize}
 \end{textblock*}
 \only<1>{\begin{textblock*}{9cm}(5.5cm, 3cm)\centering
  \includegraphics[width=5.5cm]{CombinedExtremes.png}
 \end{textblock*}}
 \only<2>{\begin{textblock*}{9cm}(5.5cm, 3cm)\centering
  \includegraphics[width=5.5cm]{HarmonieFuture.jpg}
 \end{textblock*}}
 \only<3>{\begin{textblock*}{9cm}(5.5cm, 3.75cm)\centering
  \includegraphics[width=5.5cm]{PhDStudent.png}
 \end{textblock*}}
 \only<4>{\begin{textblock*}{9cm}(5.5cm, 3cm)\centering
  \includegraphics[width=5.5cm]{GroningenHazardMap.png}
 \end{textblock*}}
\end{frame}

%\begin{frame}{Myself}
% Only mention point at the next slide
% \begin{itemize}
...
%  \item but not only point-wise because we want to borrow strength from neighbors ...
% \end{itemize}
%\end{frame}

\begin{frame}[label=StudySetting]{Study Setting}
...
\begin{frame}{RFA}
 \begin{textblock*}{11cm}(0.75cm,3cm)
 \begin{block}{Regional frequency analysis (RFA)}
  \begin{itemize}
   \item Estimate the frequency of rare events at one site by using data from several sites
   \item For instance by assuming that the marginal distributions are identical apart from a scaling factor
  \end{itemize}
 \end{block}
 \end{textblock*}

 \begin{textblock*}{11cm}(0.75cm, 6cm)
  \only<2>{$$ r_s(T) = \begin{cases}
    u_s\left(1 - \frac{\alert{\sigma_s / u_s}}{\xi}\left[1 - (\alert{\lambda_s} T)\right]^\xi\right), \qquad &\xi \neq 0, \\
    u_s\left(1 + \alert{\sigma_s / u_s} \log\left(\alert{\lambda_s} T\right)\right), \qquad &\xi = 0.
    \end{cases}$$}
  \only<3>{$$ r_s(T) = \begin{cases}
    u_s\left(1 - \frac{\alert{\gamma}}{\xi}\left[1 - (\alert{\lambda} T)\right]^\xi\right), \qquad &\xi \neq 0, \\
    u_s\left(1 + \alert{\gamma} \log\left(\alert{\lambda} T\right)\right), \qquad &\xi = 0.
    \end{cases}$$}
 \end{textblock*}
\end{frame}

\begin{frame}{Threshold selection}
 \only<1,3>{\begin{block}{Trade-off situation}
  \begin{itemize}
 ...
 \only<2>{
  ...
  KS statistic for the precipitation data in Putten as function of the non-excess probability $\tau$, together with the 95\% critical values.
 \end{textblock*}
  }
 \uncover<3>{\begin{block}{Regional scale}
  \begin{itemize}
   \item Most studies use the same quantile threshold %required in the previous setting
   \item Influence of threshold level is not often studied
  \end{itemize}
 \end{block}}
\end{frame}
...
% \begin{frame}
%  \begin{textblock*}{9cm}(1.5cm, 1.5cm)\centering
%   \includegraphics[height=7cm]{DensitiesScarrott.png}
%  \end{textblock*}
%  \begin{textblock*}{11cm}(0.75cm, 8.75cm)\centering
%  {\tiny Scarrott and MacDonald (2012): A review of extreme value threshold estimation and uncertainty quantification. \emph{REVSTAT}}
%  \end{textblock*}
% \end{frame}

\begin{frame}{Smooth splicing }
...
\begin{frame}{Weibull-GPD model}
 \begin{textblock*}{4.5cm}(1.5cm, 3cm)\centering
  \includegraphics[width=4cm]{distribution_function.pdf}
 \end{textblock*}
 \begin{textblock*}{4.5cm}(5.75cm, 3cm)\centering
  \includegraphics[width=4cm]{density.pdf}
 \end{textblock*}
 \begin{textblock*}{11cm}(0.75cm, 7.75cm)\centering
 Cdf and density of different hybrid Weibull-GPD distributions.
 %note alternative by specifying $h_1(u) = h_2(u)$
 \end{textblock*}
\end{frame}
...
\begin{frame}{Mean excess plot}
 \begin{textblock*}{9cm}(1.5cm, 3cm)\centering
  \includegraphics[width=9cm]{mean_excess.pdf}
 \end{textblock*}
 \begin{textblock*}{11cm}(0.75cm, 7.75cm)\centering
 Mean excess plot for 1 (blue), 8 (green), and 16 (red) sites.\\
 %green: 0.15, blue: 0.02, red: 0.2
 \end{textblock*}
\end{frame}

\subsection{GoF based diagnostics}
...
\begin{frame}{Effect of Bulk--Tail Transition}
 \begin{textblock*}{9cm}(1.5cm, 3cm)\centering
  \only<1>{\includegraphics[width=9cm]{AEE_5year.pdf}}
  \only<2>{\includegraphics[width=9cm]{AEE_50year.pdf}}
 \end{textblock*}
 \begin{textblock*}{11cm}(0.75cm, 7.75cm)\centering
  AEE in the \only<1>{5}\only<2>{50}-year return level as a function of the probability $\tau$ for the simulated data (blue - bulk and tail are different, red - similar). The dashed horizontal lines indicate the AEE of the selected threshold.
 \end{textblock*}
\end{frame}

\subsection{Dependent simulations}
...
\againframe{StudySetting}

\begin{frame}{Spatially averaged TS plot}
\begin{textblock*}{9cm}(1.5cm,3cm)
  \includegraphics[width=9cm]{threshold_stability_rain_data.pdf}
 \end{textblock*}
 \begin{textblock*}{11cm}(0.75cm, 7.5cm)\centering
  Threshold stability plot for the rainfall data.
  %Say something on the linear increase and the stable region afterwards
  %In particular we can reject too low thresholds
 \end{textblock*}
\end{frame}

\begin{frame}{Quantile dependence in the data}
\begin{textblock*}{9cm}(1.5cm,3cm)
  \includegraphics[width=9cm]{tail_dependence_rain_data.pdf}
 \end{textblock*}
 \begin{textblock*}{11cm}(0.75cm, 7.5cm)\centering
  Dependence in the 0.9 quantile of the rainfall data versus distance.
  %Say something that the decay with distance indicates tail independent data.
 \end{textblock*}
\end{frame}

\begin{frame}{Spatially averaged KS statistic}
 \begin{textblock*}{9cm}(1.5cm,3cm)
  \includegraphics[width=9cm]{common_KS_statistic_rain_data.pdf}
 \end{textblock*}
 \begin{textblock*}{11cm}(0.75cm, 7.5cm)\centering
  Spatially averaged KS statistic for the three different dependence models.
 \end{textblock*}
\end{frame}
